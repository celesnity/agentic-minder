...
export class AgentLanguageInterface {
  ...
  constructor(openAIAssistant?: OpenAIAssistant, geminiAssistant?: GeminiAssistant, defaultProvider: "openai" | "gemini" = "openai") {
    ...
    // FIX: Set the provider before initialization to prevent initializing the wrong service
  ...
  /**
   * Speak the given text using the current AI provider
   * This re-injects the text into the session with a request to read it aloud
   */
  public speak(text: string): void {
    if (!text) return

    print(`AgentLanguageInterface: 🗣️ Requesting speech for: "${text.substring(0, 50)}..." via ${this.currentProvider}`)

    // For both providers, we frame this as a request to read the text
    // This is necessary because we're injecting a "System/Tool" output into a "User" turn stream
    const prompt = `Please read the following text aloud to the user naturally, without adding extra commentary: "${text}"`

    if (this.currentProvider === "gemini" && this.geminiAssistant) {
      this.geminiAssistant.sendTextMessage(prompt)
    } else if (this.currentProvider === "openai" && this.openAIAssistant) {
      // For OpenAI, check if we have the method
      if (typeof this.openAIAssistant.sendMessageWithAudio === 'function') {
        this.openAIAssistant.sendMessageWithAudio(prompt)
      } else {
        print("AgentLanguageInterface: sendMessageWithAudio not available on OpenAI assistant")
      }
    } else {
      print("AgentLanguageInterface: No provider available to speak text")
    }
  }
