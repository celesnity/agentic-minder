...
export class GeminiAssistant extends BaseScriptComponent {
  ...
  createGeminiLiveSession() {
    ...
    // START FIX: Validation and Recovery for DynamicAudioOutput
    // Cast to any to perform loose checks
    const outputInput = this.dynamicAudioOutput as any

    if (typeof outputInput.initialize !== "function") {
      print(" GeminiAssistant: ⚠️ dynamicAudioOutput.initialize is missing. Attempting to recover...")

      let recoveredComponent: any = null

      // Case 1: The input is a SceneObject (has getComponent)
      if (typeof outputInput.getComponent === "function") {
        // Try finding the script by name
        recoveredComponent = outputInput.getComponent("DynamicAudioOutput")
        // If that fails, seeing as we imported the class, we could try the class type, but string is robust enough for scripts.
      }

      // Case 2: The input is another Component (has getSceneObject)
      // We check !recoveredComponent to ensure we don't overwrite if Case 1 worked
      if (!recoveredComponent && typeof outputInput.getSceneObject === "function") {
        const ownerObject = outputInput.getSceneObject()
        if (ownerObject) {
          recoveredComponent = ownerObject.getComponent("DynamicAudioOutput")
        }
      }

      // Verify the recovered component
      if (recoveredComponent && typeof recoveredComponent.initialize === "function") {
        print(" GeminiAssistant: ✅ Recovered DynamicAudioOutput component successfully.")
        // Cast back to correct type
        this.dynamicAudioOutput = recoveredComponent as DynamicAudioOutput
      } else {
        print(" GeminiAssistant: ❌ ERROR: Could not find DynamicAudioOutput script component.")
        print(" GeminiAssistant: please check the Inspector. Ensure the 'DynamicAudioOutput' input is linked to the SCRIPT COMPONENT.")
        print(" GeminiAssistant: Current input type: " + (outputInput.getTypeName ? outputInput.getTypeName() : "Unknown"))
        return
      }
    }
    // END FIX
